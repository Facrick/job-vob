...
class InterviewTabView:
    def __init__(self, controller):
        ...
        self.btn_start = primary_btn("Начать собеседование", controller.handle_start_mock,
                                     icon=ft.Icons.PLAY_ARROW)
        self.btn_reset = secondary_btn("Сбросить", controller.handle_reset_mock, icon=ft.Icons.REFRESH)

    def build(self, wide: bool = True) -> ft.Control:
        controls_bar = card(ft.Row(spacing=10, controls=[
            section_title("Тренировка собеседования с ИИ", ft.Icons.PSYCHOLOGY),
            ft.Container(expand=True), self.btn_start, self.btn_reset,
        ]), padding=12)
        ...
        return page_column([controls_bar, chat, input_bar])
