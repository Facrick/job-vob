...
class LetterAnalyzer:
    ...
    def evaluate_quiz_answer(
        ...
        Возвращает dict с ключами 'evaluation' и 'feedback'.
        Использует быструю модель для экономии и скорости.
        """
        ...
        content = self._complete(
            messages,
            temperature=0.1,  # Низкая температура для объективной оценки
            json_mode=True,
            max_tokens=400,  # Достаточно для оценки и фидбека
            model=self.analysis_model,
        )
        try:
            data = json.loads(content)
